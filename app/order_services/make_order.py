...
def make_order(db: Session, user_id: int, part_id: int, is_oem: bool = True):
    ...
    ВАЖНО: Функция НЕ вызывает db.commit().
    Коммит должен быть выполнен вызывающим кодом (колбэком).
    """
    # 1. Ищем активный заказ пользователя
    order = db.execute(
        select(Orders)
    ...
    # 2. Если заказа нет — создаём новый
    if not order:
        order = Orders(user_id=user_id)
        db.add(order)
        db.flush()  # Генерируем order.id до коммита
    ...
    # УБРАНЫ: try/except, db.commit(), db.rollback()
    return order